...
export function GastosTransacoes({
  ...
  const currentPage = parseInt(
    searchParams.get("gastos_pagina") || "1",
    10,
  );
  const setCurrentPage = (page: number) => {
    setSearchParams({ gastos_pagina: page.toString() });
  };

  const filtroCategoria = searchParams.get("gastos_categoria") || "";
  const setFiltroCategoria = (category: string) => {
    setSearchParams({ gastos_categoria: category });
  };
  const filtroDescricao = searchParams.get("gastos_descricao") || "";
  const setFiltroDescricao = (description: string) => {
    setSearchParams({ gastos_descricao: description });
  };
  const filtroEstrato = searchParams.get("gastos_estrato") || "";
  const setFiltroEstrato = (boxId: string) => {
    setSearchParams({ gastos_estrato: boxId });
  };

  // Determine mode from URL
  const isCategoryDrill = filtroCategoria !== "";
  const isFreeSearch = searchParams.get("gastos_busca") === "1";

  // Category name for header
  const categoryName =
    categories.find((c) => c.id === filtroCategoria)?.name || "";

  // Budget data for mini summary in drill mode
  const { data: budgetData } = useBudgetSummary(selectedYear, selectedMonth);
  const categoryBudget = isCategoryDrill
    ? budgetData?.budget?.find((b) => b.category.id === filtroCategoria)
    : null;

  // Local UI state
  const [showSearch, setShowSearch] = useState(
    isFreeSearch || filtroDescricao !== "",
  );
  ...
  const monthLabel = (() => {
    const monthName = new Date(2000, selectedMonth - 1, 1).toLocaleString(
      "pt-BR",
      { month: "short" },
    );
    ...
    <div className="flex flex-1 flex-col min-h-0">
      ...
      <div className="flex items-center gap-3 pb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h2 className="font-display text-lg text-foreground tracking-tight truncate">
            {isCategoryDrill ? categoryName : "Transações"}
          </h2>
        </div>
      </div>

      {/* Mini budget summary (category drill mode only) */}
      {isCategoryDrill && categoryBudget && (
        <div className="flex items-center gap-3 pb-3 px-1">
          <div className="flex-1 min-w-0">
            <div className="flex items-baseline justify-between mb-1">
              <span className="text-sm font-mono text-foreground">
                {formatMoney(categoryBudget.spent)} de{" "}
                {formatMoney(categoryBudget.amount)}
              </span>
              <span className="text-xs text-muted-foreground font-mono">
                {Math.round(categoryBudget.percentageUsed)}%
              </span>
            </div>
            <Progress
              value={Math.min(100, categoryBudget.percentageUsed)}
              filledColor={
                categoryBudget.percentageUsed > 90
                  ? "var(--color-danger)"
                  : categoryBudget.percentageUsed > 70
                    ? "var(--color-warning)"
                    : "var(--color-success)"
              }
      ...
      <div className="space-y-2 pb-3">
        ...
        {/* Row 2: Category (hidden in drill mode) + Box */}
        <div className="flex items-center gap-2">
          {/* Category chip — hidden when in category drill mode */}
          {!isCategoryDrill && (
            <div className="flex items-center">
              <Select
          ...
          <div className="flex items-center">
            ...
            >
              ...
              >
                <SelectValue>{selectedBoxName || "Estrato"}</SelectValue>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="__all__">Todas</SelectItem>
                {boxes?.map((box) => (
