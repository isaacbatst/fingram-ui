...
export const ApiProvider = ({ children }: PropsWithChildren) => {
  ...
  const authenticateWithVaultToken = useCallback(
  ...
  const authenticateWithTempToken = useCallback(async (tempToken: string) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/vault/authenticate-temp-token`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  useMemo(() => {
    if (isAuthenticated) {
      ...
      mutate((key) =>
        typeof key === "string" ? key.startsWith("transactions") : false
      ); // Refetch transactions data on authentication change
    }
  }, [isAuthenticated]);
